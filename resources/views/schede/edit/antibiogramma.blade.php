<div class="row clearfix">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="card card-collapsable">
            <div class="header">
                <h2 class="collapsable-handler">Antibiogramma</h2>
            ...
            <div class="body body-collapsable demo-masked-input" style="display: none">
                <div class="row clearfix mt-2">
                            ...
                            <select class="form-control show-tick" size="10" id="microrganismi_antibiogramma">
                                ...
                                @foreach(App\MicrorganismoPiastra::all() as $m)
                                    @if($m->microrganismo != "Nessuno")
                                        <option value="{{ $m->id }}" {{ (isset($campione) && $numeroProgressivo == 0) ? (is_selected_option(old('microrganismi_antibiogramma') ?? $campione->microantibiogramma->where('NAB',0)->first()['id_microrganismo'] ?? null,$m->id )) : '' }}>{{ $m->microrganismo }}</option>   
                                    @endif        
                                @endforeach
                            </select>
                        </div>
                    ...
                    @if(isset($campione) && $numeroProgressivo == 0) <!-- questo vale solo in fase di edit -->
                        <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2"></div>
                        <div id="form_for_antibiogramma0" class="col-lg-4 col-md-4 col-sm-4 col-xs-4 {{ (checkImmagine($campione->id,'antibiogramma0',$code) == 0) ? '' : 'hidden' }}">
                            <div class="row clearfix drop-file p-t-10">
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    <form action="{{ URL::action('ImmagineMicroAntibiogrammaController@store') }}" id="dropzone" class="dropzone dz-clickable" method="post">
                                        {{csrf_field()}}
                        ...
                        <div id="image_dropzone_antibiogramma0" class="col-lg-4 col-md-4 col-sm-4 col-xs-4 {{ (checkImmagine($campione->id,'antibiogramma0',$code) == 0) ? 'hidden' : '' }}">
                            <a href="#0" data-sub-html="Demo Description">
                                <img id="antibiogramma0" data-toggle="modal" data-target="#largeModal" alt="Immagine Antibiogramma" class="img-responsive thumbnail" @if (checkImmagine($campione->id,'antibiogramma0',$code) == 1) src="{{route('immaginiantibiogramma',[getImageMicroAntibio($campione->id,'antibiogramma0'),$campione->id])}}" @else src="{{route('immaginitemporaneepiastre',[getImageTemporary($code,'antibiogramma0')]) }}" @endif>
                            </a>
                        </div>
                        <div class="row clearfix">
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 align-right">
                                <input type="button" tipo="antibiogramma0" name="elimina_foto_conta" id="elimina_foto_conta" class="btn bg-blue btn waves-effect" data-toggle="modal" data-target="#eliminaFotoModal" value="Elimina Foto" {{ isset($campione) && $campione->bloccato == 1 ? 'disabled' : '' }} >    
                            </div>
                        </div>
                    @else
                    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2"></div>
                ...
                @if(isset($campione))
                    ...
                    @for($i = 1; $i <= $occorrenza; $i++ )
                        ...
                        @include('schede.edit.antibiogrammi_campione',['occorrenza' => $i])
                    @endfor
                @endif
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 pl-0">
                                            ...
                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                ...
                                                <div class="row clearfix">
                                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 align-left">
                                                        <input type="button" id="aggiungi_antibiotico_resistenza" class="btn btn-primary waves-effect" value="Aggiungi" disabled {{ isset($campione) && $campione->bloccato == 1 ? 'disabled' : '' }} >
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                ...
                                                <div class="row clearfix">
                                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 pl-1-2 align-left">
                                                        <input type="button" id="cancella_antibiotico_resistenza" class="btn btn-danger waves-effect" value="Cancella" disabled {{ isset($campione) && $campione->bloccato == 1 ? 'disabled' : '' }} >
                                                    </div>
                                                </div> 
                                            </div>
                                        </div>
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
                            <div class="row clearfix drop-file p-t-10">
                                ...
                                <div id="image_dropzone" class="col-lg-12 col-md-12 col-sm-12 col-xs-12 mb-0 hidden">
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                        <a href="#0" data-sub-html="Demo Description">
                                ...
                                <div id="button_dropzone" class="row clearfix mr-1-2 hidden">
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 align-right">
                                        <input type="button" tipo="" name="elimina_foto_antibiogramma" id="elimina_foto_antibiogramma" class="btn bg-blue btn waves-effect" data-toggle="modal" data-target="#eliminaFotoModal" value="Elimina Foto" {{ isset($campione) && $campione->bloccato == 1 ? 'disabled' : '' }} >    
                                    </div>
                                </div>
                            </div>
                        </div>
